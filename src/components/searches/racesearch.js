...
export default function RaceSearch({raceToSearch}){
    ...
    useEffect(() => {
        ...
        .then(response => {
            ...
            setAbilityScoreImprovements(response.data.asi.filter(ability => ability.value > 0).map(ability => ({
                attribute:ability.attributes,
                value:ability.value
            })))

            if(response.data.subraces) { //only set them if there are subraces\
                setSubraces(response.data.subraces.map(subrace => ({
                    name:subrace.name,
                    asi:{attribute:subrace.asi[0].attributes, value:subrace.asi[0].value},
                    asi_desc:subrace.asi_desc,
                    traits:subrace.traits,
                    desc:subrace.desc
                })))
                console.log('subrace asi from api: ', response.data.subraces[0].asi)
                console.log(response.data.subraces[0].asi[0].attributes, ' coloncoloncolon')
                console.log(response.data.subraces[0].asi[0].value, ' theyre taking the hobbits to Isengard-gard-gard-gard-gard')
            } else {
                setSubraces([])
            }
    ...
    return(
        ...
        <div>
            ...
            {abilityScoreImprovements.map((ability) => {
                return <li key={ability.attribute}>{ability.attribute}: {ability.value}</li>})}
            </ul>
            <h2>Vision</h2>
            ...
            <h2>Size</h2>
            <p>{raceResult?.size_raw}: {raceResult?.size}</p>
            <h2>Age</h2>
            <p>{raceResult?.age}</p>
            <h2>Alignment</h2>
